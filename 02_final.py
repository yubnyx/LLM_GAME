...
if "initialized" not in st.session_state:
    ...
    st.session_state.reaction_shown = False
    ...
    if 1 <= st.session_state.round <= 3:
        ...
        if not st.session_state.story_selected:
            ...
            if st.button("이야기 선택하기"):
                ...
                # 기사 반응 및 미터기 증가를 딱 한 번만 처리!
                selected_idx = [opt["story"] for opt in st.session_state.current_story_options].index(story)
                ...
                st.session_state.reaction_shown = True
                st.rerun()


        if st.session_state.story_selected and st.session_state.reaction_shown:
            st.markdown("#### 나는 이야기를 시작했다...")
            time.sleep(5)
            personality = DRIVER_PERSONALITY[st.session_state.driver_num]
            if os.path.exists(personality["image"]):
                st.image(personality["image"], width=300)
            st.write(f"**기사 반응**: {st.session_state.reaction}")
